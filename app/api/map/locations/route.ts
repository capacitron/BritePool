...
export async function GET(request: NextRequest) {
  try {
    ...
    logError(error, { action: 'fetch_map_locations' })
    return NextResponse.json({ error: 'Failed to fetch map locations' }, { status: 500 })
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    logError(error, { action: 'create_map_location' })
    ...
    return NextResponse.json({ error: 'Failed to create map location' }, { status: 500 })
  }
}
