...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    // Fetch forum posts
    const [forumPosts, forumTotal] = type === 'media' ? [[], 0] : await Promise.all([
    ...
    // Fetch media items
    const [mediaItems, mediaTotal] = type === 'forum_post' ? [[], 0] : await Promise.all([
    ...
    const totalItems = type === 'all' ? forumTotal + mediaTotal : type === 'forum_post' ? forumTotal : mediaTotal;
