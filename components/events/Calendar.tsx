...
export function Calendar<T extends EventData = EventData>({ events, onDayClick, onEventClick }: CalendarProps<T>) {
  ...
  const renderCalendarDays = () => {
    ...
    for (let i = 0; i < firstDayOfMonth; i++) {
      days.push(
        <div key={`empty-${i}`} className="h-24 bg-sand-50 border border-sand-200" />
      )
    }
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      days.push(
        <div
          ...
          className={cn(
            'h-24 p-1 border border-sand-200 cursor-pointer hover:bg-sand-100 transition-colors overflow-hidden',
            isToday && 'bg-forest-50 ring-2 ring-forest-500 ring-inset'
          )}
        >
          <div className={cn(
          ...
          <div className="space-y-0.5">
            {dayEvents.slice(0, 3).map(event => (
              <div
                ...
                className={cn(
                  ...
                  EVENT_TYPE_COLORS[event.type] || 'bg-forest-400'
                )}
                title={event.title}
              >
                {event.title}
            ...
            {dayEvents.length > 3 && (
              <div className="text-xs text-forest-500 px-1">
  ...
  return (
    <div className="bg-white rounded-lg border border-sand-200 shadow-warm">
      ...
      <div className="grid grid-cols-7">
        {DAYS.map(day => (
          <div
            key={day}
            className="p-2 text-center text-sm font-medium text-forest-600 bg-sand-100 border-b border-sand-200"
          >
            {day}
          </div>
        ))}
